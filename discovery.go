...
// newHTTPClient returns the client used for discovery and userinfo. stdlib
// net/http, per the fleet's Go stack canon.
...
//
// Discovery is done by hand rather than with an OIDC relying-party library:
// this package is not a relying party. It never runs a code flow, holds no
// client credentials and mints no sessions — it verifies a token somebody else
// obtained and, for display claims the access token omits, asks userinfo. One
// well-known document and two fields is the whole requirement, and staying
// library-free keeps the package un-bound to any provider's SDK.
