...
// at pulls jwx v1 back into the module graph, which is a worse trade than these
// sixty lines.
...
const (
	...
	// enough that a service is not a load generator against the provider.
	...
	// stops a compromised or confused endpoint from feeding a stream.
	keySetMaxBytes = 1 << 20
)

// keySet holds the issuer's public keys and refreshes them in the background.
//
// A refresh failure is deliberately not fatal: the previous key set stays in
// place and requests keep verifying. Dropping the keys would turn a blip at the
// identity provider into an outage of a service whose whole job may be to still
